...
.ORIGINALPROJECTURI https://github.com/asheroto/winget-install
...
[Version 3.1.0] - Added support for one-line installation with irm and Invoke-Expression compatible with $Force session variable. Added UpdateSelf command to automatically update the script to the latest version. Created short URL asheroto.com/winget.
...
[Version 5.2.1] - Fixed 2 bugs:
1. Fixed [{
	"code": "PSUseApprovedVerbs",
	"severity": 4,
	"message": "The cmdlet 'Handle-Error' uses an unapproved verb.",
...
2. Fixed [{
	"code": "PSUseApprovedVerbs",
	"severity": 4,
	"message": "The cmdlet 'Path-ExistsInEnvironment' uses an unapproved verb.",
	"source": "PSScriptAnalyzer",
	"startLineNumber": 733,
    "endLineNumber": 745,
}]
...
function Invoke-ErrorHandler {
    <#
        ...
        .EXAMPLE
            ...
            This example shows how you might use the Invoke-ErrorHandler function in a try-catch block.
            If an error occurs in the try block, the catch block catches it and calls Invoke-ErrorHandler,
...
function Test-PathInEnvironment {
    ...
    .EXAMPLE
    Test-PathInEnvironment -PathToCheck "C:\Program Files\MyApp" -Scope 'User'
    #>
...
try {
    ...
    if ($osVersion.NumericVersion -ne 2019 -and $AlternateInstallMethod -eq $false -and $RunAsSystem -eq $false) {
        ...
        try {
            ...
            $errorHandled = Invoke-ErrorHandler $_
