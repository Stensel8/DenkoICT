...
Initialize-DeploymentScript -LogName 'Install-Drivers.log' -RequiredModules @('Logging','System','Registry','Winget') -RequireAdmin
...
try {
    ...
    # Ensure WinGet (for vendor tools via winget)
    $winget = $null
    try { $winget = Test-WinGetFunctional } catch { Write-Log "WinGet not functional: $($_.Exception.Message)" -Level Warning }
